...
-- Deactivated for SplendidDataTest: CREATE PUBLICATION regress_pub_for_allsequences_alltables FOR ALL SEQUENCES, ALL TABLES, ALL TABLES;

-- fail - Specifying ALL SEQUENCES more than once
-- Deactivated for SplendidDataTest: CREATE PUBLICATION regress_pub_for_allsequences_alltables FOR ALL SEQUENCES, ALL TABLES, ALL SEQUENCES;
...
SET client_min_messages = 'ERROR';
-- Deactivated for SplendidDataTest: CREATE PUBLICATION testpub_syntax3 FOR TABLES IN SCHEMA testpub_rf_schema1 WHERE (a = 123);
-- Deactivated for SplendidDataTest: CREATE PUBLICATION testpub_syntax3 FOR TABLES IN SCHEMA testpub_rf_schema1, testpub_rf_schema1 WHERE (a = 123);
RESET client_min_messages;
...
-- is not specified
-- Deactivated for SplendidDataTest: CREATE PUBLICATION testpub_forschema1 FOR CURRENT_SCHEMA;

-- check create publication on CURRENT_SCHEMA along with FOR TABLE
-- Deactivated for SplendidDataTest: CREATE PUBLICATION testpub_forschema1 FOR TABLE CURRENT_SCHEMA;
...
-- Deactivated for SplendidDataTest: ALTER PUBLICATION testpub1_forschema ADD TABLES IN SCHEMA foo, bar (a, b);
...
-- Deactivated for SplendidDataTest: CREATE PUBLICATION testpub_error FOR pub_test2.tbl1;
