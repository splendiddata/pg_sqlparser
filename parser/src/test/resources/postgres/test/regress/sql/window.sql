...
-- Deactivated for SplendidDataTest: SELECT * FROM rank() OVER (ORDER BY random());
...
-- Deactivated for SplendidDataTest: SELECT rank() OVER (PARTITION BY four, ORDER BY ten) FROM tenk1;
...
SELECT * FROM
  (SELECT empno,
          depname,
          salary,
          count(1) OVER (RANGE BETWEEN CURRENT ROW AND CURRENT ROW) c
   FROM empsalary) emp
WHERE c = 1;
...
SELECT * FROM
  (SELECT empno,
          depname,
          salary,
          count(1) OVER (ROWS BETWEEN CURRENT ROW AND CURRENT ROW) c
   FROM empsalary) emp
WHERE c > 1;
